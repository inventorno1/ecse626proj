...
out_dir = "/cim/ehoney/ecse626proj/experiment9_2"
...
optimizer = AdamW(chain(model.parameters(), slice_embedder.parameters()), lr=lr)
...
train_dataset = ADNIDatasetSlicesInOrder(data_dir, n=slices_at_once, return_target_slice_index=True, symmetric_normalisation=True)
...
for epoch in range(epoch_start, epochs):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        slice_embedding = slice_embedder(first_target_slice_indices//slices_at_once)
        slice_embedding = slice_embedding.unsqueeze(1)
        # print(f'slice_embedding - dtype: {slice_embedding.dtype}, device: {slice_embedding.device}, shape: {slice_embedding.shape}')
        ...
        predicted_noise = model(input_tensor, timesteps, slice_embedding).sample
    ...
    checkpoint = {
        ...
        'embedder_sd': slice_embedder.state_dict(),
